...
#      $log->debug('AFTER_DISPATCH CALLED');
  ...
  $self->hook('after_render' => sub {
      ...
      do { $log->debug('Compressiion not required'); return 1 }
...
#    $v->types->type('csv' => 'text/csv');
  ...
  $r->get('/mosrides' => sub {
    ...
    RIDEPAGE: while ($iSkip<=abs($#skip)) {
      $srcUrl=$baseUrl.(@skip?'?skip='.$skip[$iSkip++]:'');
#      $log->debug('get url="'.$srcUrl.'"');
      my $htmlRides=$ua->get($srcUrl)->decoded_content;
      @skip=sort { $a <=> $b } keys {map { $_=>1 } $htmlRides=~m%\?skip=(\d+)"%sg} unless @skip;
      while ($htmlRides=~m/<a href="(\/rides\/(\d{4}\/\d{2}\/\d{2})\/\d+)"\s*>\s*([^<]+)\s*<\/a>.*?(\d+:\d+)[^<]*?<\/span>.+?<a\s+href="([^"]+)">Участвуют<\/a>.*?<span[^>]*>\s*(\d+)\s*<\/span>/sg) {
